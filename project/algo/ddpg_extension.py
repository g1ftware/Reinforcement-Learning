...
class DDPGExtension(DDPGAgent):
    ...
    def compute_lnss_reward(self, batch):
        """
        Compute LNSS surrogate rewards for N-step returns.
        """
        gamma = self.gamma
        N = self.cfg.n_step  # N-step parameter from config

        rewards = batch.reward.cpu().numpy()
        not_done = batch.not_done.cpu().numpy()
        ...
        for t in range(len(rewards)):
            ...
            for n in range(N):
                if t + n < len(rewards):
                    lnss_reward += discount * rewards[t + n]
                    normalization += discount
                    discount *= gamma
                    if not_done[t + n] == 0:  # Terminal state
                        break
            lnss_rewards.append(lnss_reward / normalization)
    
        return torch.tensor(lnss_rewards, dtype=torch.float32).unsqueeze(1).to(self.device)


    def _update(self):
        batch = self.buffer.sample(self.batch_size, device=self.device)
        
        # Compute LNSS surrogate rewards
        lnss_reward = self.compute_lnss_reward(batch)
